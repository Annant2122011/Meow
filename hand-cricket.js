...
const bossInfo = [
    { name: "The Rookie", icon: "🟢", color: "gray", desc: "Plays entirely randomly. A good warmup.", taunts: { wkt: ["Oops, did I do that?", "Beginner's luck!"], six: ["Wow, I hit it far!", "That felt good!"] } },
    { name: "The Wall", icon: "🧱", color: "#00d2ff", desc: "Defends heavily. Hard to score boundaries against.", taunts: { wkt: ["Impenetrable.", "You cannot break the wall.", "Patience is key."], six: ["A rare breach.", "Calculated risk."] } },
    { name: "The Slogger", icon: "🏏", color: "orange", desc: "Highly aggressive. Throws massive numbers.", taunts: { wkt: ["A minor setback!", "I swing for the fences!"], six: ["OUT OF THE STADIUM!", "HAVE SOME OF THAT!", "TOO EASY!"] } },
    { name: "The Illusionist", icon: "🎭", color: "#b91c1c", desc: "Throws opposites. If you go high, he goes low.", taunts: { wkt: ["Sleight of hand!", "Look over there!"], six: ["An illusion broke...", "Tricky!"] } },
    { name: "The Copycat", icon: "🪞", color: "#00bfa5", desc: "Mirrors your previous move perfectly.", taunts: { wkt: ["Stop hitting yourself!", "I'm just like you!"], six: ["I let you have that one.", "Copying is hard..."] } },
    { name: "The Gambler", icon: "🎲", color: "#facc15", desc: "Takes wild guesses. Brilliant or awful.", taunts: { wkt: ["Jackpot!", "Snake eyes for you!"], six: ["Bust!", "I'll double down next time!"] } },
    { name: "The Mathematician", icon: "🧮", color: "#9333ea", desc: "Calculates the weighted average of your throws.", taunts: { wkt: ["Statistically inevitable.", "The numbers don't lie."], six: ["A statistical anomaly.", "Margin of error..."] } },
    { name: "The Sniper", icon: "🎯", color: "#ff2a2a", desc: "Reads lifetime stats. Highly predictive.", taunts: { wkt: ["Predictable.", "Right in my crosshairs.", "I knew you'd throw that."], six: ["Precision strike.", "Calculated."] } },
    { name: "Grandmaster", icon: "🧙‍♂️", color: "#00ff88", desc: "Reads CURRENT match patterns. Learns in real-time.", taunts: { wkt: ["Checkmate.", "Your patterns betray you.", "I am three moves ahead."], six: ["Flawless execution.", "Masterclass."] } },
    { name: "Cricket God", icon: "👑", color: "#fbbf24", desc: "The ultimate challenge. Near-perfect prediction.", taunts: { wkt: ["Bow to the God.", "You cannot defy destiny."], six: ["A mere mortal strikes?", "Blasphemy!"] } }
];
...
const shopItems = {
    ...
    coins: [
        ...
        { id: 'copper', name: 'Copper Coin', price: 500, icon: '🟤' },
        { id: 'silver', name: 'Silver Coin', price: 1500, icon: '⚪' },
        { id: 'gold', name: 'Gold Coin', price: 3000, icon: '🟡' },
        { id: 'bitcoin', name: 'Crypto Coin', price: 5000, icon: '₿' },
        { id: 'diamond', name: 'Diamond Coin', price: 10000, icon: '💎' }
    ]
};

let tossData = { caller: null, call: null, result: null };
let currentUser = null; 
let srChartInstance = null; let runsChartInstance = null; let throwDnaInstance = null; let fatalChartInstance = null; let wormChartInstance = null;
const handEmojis = { 0: '🛡️', 1: '☝️', 2: '✌️', 3: '🤟', 4: '🖖', 5: '🖐️', 6: '👍' };

const tossStep1 = document.getElementById('toss-step-1'); const tossStep2 = document.getElementById('toss-step-2'); const tossChoiceText = document.getElementById('toss-choice-text'); const tossResultScreen = document.getElementById('toss-result-screen'); const playerDecisionBox = document.getElementById('player-decision-box'); const computerDecisionBox = document.getElementById('computer-decision-box'); const matchScreen = document.getElementById('match-screen'); const tossScreen = document.getElementById('toss-screen'); const setupScreen = document.getElementById('setup-screen'); const inningsStatus = document.getElementById('innings-status'); const commentaryBox = document.getElementById('hand-commentary'); const targetBox = document.getElementById('target-box'); const targetScoreUi = document.getElementById('target-score'); const actionArea = document.getElementById('hand-action-area'); const zeroBtn = document.getElementById('zero-btn'); 
...
window.onload = function() {
    ...
    if (isProfilePage || isTournamentPage) {
        ...
        currentUser = storedUser; syncUserData(currentUser);
        applyCosmetics(); 
        if (isProfilePage) { renderProfilePage(); renderShop(); }
        if (isTournamentPage) renderTournamentPage();
    } else {
        if (storedUser) { loadUser(storedUser); } 
        else { const loginModal = document.getElementById('login-modal'); if (loginModal) loginModal.style.display = 'flex'; }
    }
};
...
function syncUserData(username) {
    ...
    u.xp = u.xp || 0; u.tournamentLevel = u.tournamentLevel || 0;
    ...
    if (!u.last10SR) u.last10SR = []; if (!u.last20Innings) u.last20Innings = []; 
...
function applyCosmetics() {
    ...
    if (coinHeads && coinTails) {
        ...
        if (u.equippedCoin !== 'default') {
            coinHeads.classList.add('coin-' + u.equippedCoin); coinTails.classList.add('coin-' + u.equippedCoin);
        }
        if (u.equippedCoin === 'bitcoin') { coinHeads.innerHTML = '₿'; coinTails.innerHTML = '₿'; } 
        else { coinHeads.innerHTML = 'HEADS'; coinTails.innerHTML = 'TAILS'; }
    }
}
...
function loginUser() {
    ...
    syncUserData(username); localStorage.setItem('hc_currentUser', username); loadUser(username);
}
...
function applyRankUI(username, avatarBoxId) {
    ...
    const xp = usersDB[username].xp || 0; const rank = getRankDetails(xp);
    const avatarBox = document.getElementById(avatarBoxId);
    if(avatarBox) { avatarBox.className = ''; avatarBox.classList.add(rank.class); }
    return { rank, xp };
}

function loadUser(username) {
    currentUser = username; syncUserData(username);
    const loginModal = document.getElementById('login-modal'); const profileBtn = document.getElementById('user-profile-btn'); 
    ...
    if (activeBoss !== null) {
        gameState.isTournament = true; gameState.currentBoss = parseInt(activeBoss);
        const boss = bossInfo[gameState.currentBoss];
        
        gameState.maxWickets = 3; gameState.maxBalls = 30;
        
        if (setupScreen) setupScreen.style.display = 'none';
        const sub = document.querySelector('.game-subtitle');
        if (sub) { sub.innerText = `VS ${boss.icon} ${boss.name}`; sub.style.color = boss.color; sub.style.fontWeight = "bold"; }
    ...
    const buildSection = (items, typeStr, unlockedArr, equippedId) => {
        ...
        items.forEach(item => {
            ...
            html += `<div class="shop-item ${isEquipped ? 'equipped' : ''}">
                        <div class="shop-item-icon">${item.icon || item.id}</div>
...
function renderTournamentPage() {
    const usersDB = JSON.parse(localStorage.getItem('hc_usersDB')) || {}; const stats = usersDB[currentUser];
    const tLevel = stats.tournamentLevel || 0;
    
    let html = '<div class="boss-grid">';
    bossInfo.forEach((boss, i) => {
        let statusClass = 'boss-locked'; let actionHtml = `<span class="locked-badge">🔒 LOCKED</span>`;
        if (i < tLevel) { 
        ...
        html += `<div class="boss-card ${statusClass}" style="border-left: 5px solid ${boss.color};"><div class="boss-icon">${boss.icon}</div><div class="boss-info"><div class="boss-name" style="color: ${i === tLevel ? boss.color : 'white'};">${boss.name}</div><div class="boss-desc">${boss.desc}</div></div><div>${actionHtml}</div></div>`;
    });
    html += '</div>'; document.getElementById('boss-grid-container').innerHTML = html;
}

function startBossFight(index) { localStorage.setItem('hc_tourney_boss', index); window.location.href = 'index.html'; }
...
function renderProfilePage() {
    ...
    const rText = document.getElementById('prof-rank'); if(rText) { rText.innerText = rankData.rank.title; rText.className = ''; rText.classList.add(rankData.rank.class); }
    
    const xText = document.getElementById('prof-xp'); if(xText) xText.innerText = rankData.xp;
    const cText = document.getElementById('prof-coins'); if(cText) cText.innerText = stats.coins;

    document.getElementById('prof-matches').innerText = stats.matches; document.getElementById('prof-wins').innerText = stats.wins; document.getElementById('prof-losses').innerText = stats.losses; document.getElementById('prof-ties').innerText = stats.ties; document.getElementById('prof-total-runs').innerText = stats.totalRuns; document.getElementById('prof-total-wickets').innerText = stats.totalWicketsTaken; document.getElementById('prof-hs').innerText = stats.highestScore; document.getElementById('prof-ducks').innerText = stats.ducks;
    let batAvg = "0.00"; if (stats.matches > 0) batAvg = (stats.totalRuns / stats.matches).toFixed(2); document.getElementById('prof-bat-avg').innerText = batAvg;
    const wickets = stats.totalWicketsTaken || 0; document.getElementById('prof-bowl-avg').innerText = wickets > 0 ? (stats.totalRunsConceded / wickets).toFixed(2) : "-";
    const avgSR = stats.totalBallsFaced > 0 ? ((stats.totalRuns / stats.totalBallsFaced) * 100).toFixed(2) : "0.00"; const oversBowled = stats.totalBallsBowled / 6; const avgEco = oversBowled > 0 ? (stats.totalRunsConceded / oversBowled).toFixed(2) : "0.00"; document.getElementById('prof-sr').innerText = avgSR; document.getElementById('prof-eco').innerText = avgEco;
    ...
    if (achContainer) achContainer.innerHTML = achHtml;

    if(srChartInstance) srChartInstance.destroy(); if(runsChartInstance) runsChartInstance.destroy(); if(throwDnaInstance) throwDnaInstance.destroy(); if(fatalChartInstance) fatalChartInstance.destroy();
    ...
    if (dnaCtxElement && stats.battingThrows) { 
        ...
        throwDnaInstance = new Chart(dnaCtxElement.getContext('2d'), { 
            ...
            options: { plugins: { legend: { display: false } }, scales: { r: { min: 0, beginAtZero: true, angleLines: { color: 'rgba(255,255,255,0.1)' }, grid: { color: 'rgba(255,255,255,0.1)' }, pointLabels: { color: '#fff', font: {size: 14, family: "'Orbitron', sans-serif"} }, ticks: { display: false } } } } 
        }); 
    }
...
function switchTab(tabId) { document.querySelectorAll('.tab-btn').forEach(btn => { btn.classList.remove('active-tab'); }); document.querySelectorAll('.tab-content').forEach(content => { content.classList.remove('active-content'); }); const eventObj = window.event; if (eventObj && eventObj.target) { eventObj.target.classList.add('active-tab'); } const activeContent = document.getElementById(tabId); if (activeContent) { activeContent.classList.add('active-content'); } }
function fireConfetti() { if (typeof confetti !== 'undefined') { var duration = 3000; var end = Date.now() + duration; (function frame() { confetti({ particleCount: 5, angle: 60, spread: 55, origin: { x: 0 }, colors: ['#00ff88', '#00d2ff'] }); confetti({ particleCount: 5, angle: 120, spread: 55, origin: { x: 1 }, colors: ['#00ff88', '#00d2ff'] }); if (Date.now() < end) { requestAnimationFrame(frame); } }()); } }
function showToast(message) { const container = document.getElementById('toast-container'); if (!container) return; const toast = document.createElement('div'); toast.className = 'toast'; toast.innerText = message; container.appendChild(toast); setTimeout(() => { if (container.contains(toast)) { container.removeChild(toast); } }, 5000); }
function updateAtmosphere() { const body = document.body; if (gameState.innings === 2 && gameState.isPlayerBatting && !gameState.gameOver) { const runsNeeded = gameState.target - gameState.playerStats.runs; const ballsLeft = gameState.maxBalls - gameState.playerStats.balls; if (runsNeeded > 0 && runsNeeded <= 15 && ballsLeft <= 12) { body.classList.add('danger-pulse'); return; } } body.classList.remove('danger-pulse'); }

// --- GAME UI & SETUP ---
function setFormat(wickets, balls, btnId) { gameState.maxWickets = wickets; gameState.maxBalls = balls; document.querySelectorAll('.setup-btn').forEach(btn => { if (btn.id && btn.id.startsWith('btn-fmt')) { btn.classList.remove('active-setup-btn'); } }); const activeBtn = document.getElementById(btnId); if (activeBtn) { activeBtn.classList.add('active-setup-btn'); } }
function setDifficulty(level, btnId) { gameState.aiDifficulty = level; document.querySelectorAll('.setup-btn').forEach(btn => { if (btn.id && btn.id.startsWith('btn-diff')) { btn.classList.remove('active-setup-btn'); } }); const activeBtn = document.getElementById(btnId); if (activeBtn) { activeBtn.classList.add('active-setup-btn'); } }
...
function goToToss() {
    ...
    if (coin) { coin.style.transition = 'none'; coin.style.transform = 'rotateY(0deg)'; }
    ...
    if (tossData.caller === 'player') {
        ...
        pControls.style.display = 'flex'; cControls.style.display = 'none';
    } else {
        ...
        pControls.style.display = 'none'; cControls.style.display = 'flex';
    }
}

function callCoin(choice) { tossData.call = choice; document.getElementById('player-call-controls').style.display = 'none'; document.getElementById('toss-status-text').innerHTML = `You called: <span style='color: var(--accent-blue); text-transform: uppercase;'>${choice}</span>`; executeCoinFlip(); }
...
function processTossResult() {
    ...
    const playerWins = (tossData.caller === 'player' && tossData.call === tossData.result) || (tossData.caller === 'comp' && tossData.call !== tossData.result);
    gameState.commentaryHistory.push(`🪙 TOSS: It's ${tossData.result.toUpperCase()}! ${playerWins ? 'You won the toss!' : 'Computer won the toss.'}`);

    if (playerWins && currentUser) { let uDB = JSON.parse(localStorage.getItem('hc_usersDB')); uDB[currentUser].tossesWon += 1; localStorage.setItem('hc_usersDB', JSON.stringify(uDB)); }
    ...
    if (playerWins) {
        winText.innerText = "🎉 YOU WON THE TOSS!"; winText.style.color = 'var(--accent-neon)'; 
        pDecision.style.display = 'block'; cDecision.style.display = 'none';
    } else {
        winText.innerText = "🤖 COMPUTER WON THE TOSS!"; winText.style.color = 'var(--accent-red)';
...
function startMatch(playerOptsToBat) { gameState.isPlayerBatting = playerOptsToBat; gameState.commentaryHistory.push(`👤 You elected to ${playerOptsToBat ? 'BAT' : 'BOWL'} first.`); continueToMatch(); }

function continueToMatch() { 
    if (tossScreen) tossScreen.style.display = 'none'; 
    matchScreen.style.display = 'block'; updateMatchUI(); 
...
function updateMatchUI() {
    ...
    if (gameState.isTournament) {
        const boss = bossInfo[gameState.currentBoss];
        if (cLabel) { cLabel.innerText = `${boss.icon} ${boss.name}`; cLabel.style.color = boss.color; }
        if (cScoreLabel) { cScoreLabel.innerText = `${boss.name} SCORE`; cScoreLabel.style.color = boss.color; }
    }
    ...
    if (zeroBtn) { if (gameState.isPlayerBatting) { zeroBtn.innerHTML = "🛡️ 0 (DEFEND)"; zeroBtn.style.background = "rgba(0, 210, 255, 0.1)"; zeroBtn.style.borderColor = "var(--accent-blue)"; } else { zeroBtn.innerHTML = "↔️ 0 (WIDE)"; zeroBtn.style.background = "rgba(255, 42, 42, 0.1)"; zeroBtn.style.borderColor = "var(--accent-red)"; } }
    updateAtmosphere();
}
...
function getBossThrow(bossIndex) {
    ...
    let getLikely = (throwsObj) => {
        let maxT = '1', maxV = -1;
        for(let k in throwsObj) { if(throwsObj[k] > maxV) { maxV = throwsObj[k]; maxT = k; } }
    ...
    let snipe = (target) => {
        if(isCompBatting) {
            ...
            while(safe === target) safe = Math.floor(Math.random() * 6) + 1;
            return safe;
        } else { return target; }
    };
    ...
    switch(bossIndex) {
        ...
        case 1: // Wall
            if (isCompBatting) return [0, 0, 1, 1, 2, 2, 3, 4][Math.floor(Math.random() * 8)];
            else return [1, 2, 3, 4, 0][Math.floor(Math.random() * 5)];
        case 2: // Slogger
            if (isCompBatting) return [4, 4, 5, 5, 6, 6, 3][Math.floor(Math.random() * 7)];
            else return [4, 5, 6, 1, 2][Math.floor(Math.random() * 5)];
        case 3: // Illusionist
        ...
        case 4: // Copycat
            if (gameState.playerHistory.length > 0) return gameState.playerHistory[gameState.playerHistory.length - 1];
            return Math.floor(Math.random() * 6) + 1;
        case 5: // Gambler
            if (Math.random() < 0.3) { let currT = getLikely(pMatchThrows); return snipe(currT); }
            return Math.floor(Math.random() * 6) + 1;
        case 6: // Mathematician
            let hist = gameState.playerHistory.slice(-5);
            if(hist.length === 0) return Math.floor(Math.random() * 6) + 1;
...
function getComputerThrow() { 
    if (gameState.isTournament) return getBossThrow(gameState.currentBoss); 
    if (gameState.compConsecZeros >= 2) return Math.floor(Math.random() * 6) + 1; 
...
function playHand(playerNum) {
    ...
    gameState.playerHistory.push(playerNum); // Record for AI tracking

    if (gameState.isPlayerBatting) { gameState.playerMatchBatting[playerNum] = (gameState.playerMatchBatting[playerNum] || 0) + 1; } 
    else { gameState.playerMatchBowling[playerNum] = (gameState.playerMatchBowling[playerNum] || 0) + 1; }
...
function handleWide(batterNum) {
    ...
    currentBatterStats.runs += runsToAdd; currentBatterStats.extras += runsToAdd; currentBatterStats.currentWicketRuns += runsToAdd;
...
function handleRuns(runs) {
    const currentBatterStats = gameState.isPlayerBatting ? gameState.playerStats : gameState.compStats;
    currentBatterStats.runs += runs; currentBatterStats.balls++; currentBatterStats.currentWicketRuns += runs;
    currentBatterStats.wormData.push({ ball: currentBatterStats.balls, runs: currentBatterStats.runs, wkt: false });
    
    let tType = null; 
    if (!gameState.isPlayerBatting && runs === 6) tType = "six"; 
...
function drawWormChart() {
    ...
    let pData = Array(maxB + 1).fill(null); let pRadii = Array(maxB + 1).fill(0);
    gameState.playerStats.wormData.forEach(d => { pData[d.ball] = d.runs; if (d.wkt) pRadii[d.ball] = 5; });
    for (let i=1; i<=gameState.playerStats.balls; i++) { if (pData[i] === null) pData[i] = pData[i-1]; }
    
    let cData = Array(maxB + 1).fill(null); let cRadii = Array(maxB + 1).fill(0);
    gameState.compStats.wormData.forEach(d => { cData[d.ball] = d.runs; if (d.wkt) cRadii[d.ball] = 5; });
    for (let i=1; i<=gameState.compStats.balls; i++) { if (cData[i] === null) cData[i] = cData[i-1]; }
    ...
    wormChartInstance = new Chart(ctxElement.getContext('2d'), { 
        ...
        options: { 
            ...
            scales: { y: { beginAtZero: true, grid: {color: 'rgba(255,255,255,0.1)'} }, x: { title: { display: true, text: 'Balls', color: '#a1a1aa' }, grid: {color: 'rgba(255,255,255,0.1)'} } }, 
            color: '#fff' 
        } 
    });
}
...
function endGame(result) {
    gameState.gameOver = true; actionArea.style.display = 'none'; 
    const endControls = document.getElementById('end-game-controls'); endControls.style.display = 'flex'; 
    ...
    if (gameState.isTournament) {
        ...
        if (result === "PLAYER_WINS" && stats.tournamentLevel === gameState.currentBoss) {
            stats.tournamentLevel++; localStorage.setItem('hc_usersDB', JSON.stringify(usersDB));
        ...
        if (playAgainBtn) { playAgainBtn.innerText = "🔙 TO GAUNTLET"; playAgainBtn.onclick = function() { localStorage.removeItem('hc_tourney_boss'); window.location.href = 'tournament.html'; }; }
    }
}

function evaluateAchievements(stats) {
    if (!stats.achLevels) stats.achLevels = {};
    ...
    achievementList.forEach(ach => {
        ...
        let level = 0;
        for(let i=0; i<ach.thresholds.length; i++) {
            if (val >= ach.thresholds[i]) level = i + 1;
            else break;
        }
...
function saveLifetimeStats(result) {
    ...
    let matchXP = 50; if (result === "PLAYER_WINS") { matchXP += 100; }
    matchXP += gameState.playerStats.runs; matchXP += (gameState.compStats.wicketsLost * 10);
    ...
    if (gameState.compStats.wicketsLost >= 10) stats.tenWicketHauls = (stats.tenWicketHauls || 0) + 1;
    ...
    stats.lifetimeCoins = (stats.lifetimeCoins || 0) + matchCoins;
    ...
    if (gameState.compStats.wicketsLost > 0) {
        let currentWkts = gameState.compStats.wicketsLost; let currentRuns = gameState.compStats.runs;
...
function generateAIInsight(result) {
    ...
    if (gameState.isTournament) { insightBox.innerText = `Boss Fight Completed. ${result === 'PLAYER_WINS' ? 'You mastered their technique!' : 'Analyze their unique Throw DNA and try again.'}`; } 
    else if (gameState.aiDifficulty === 'hard') { insightBox.innerText = "Pro AI Engine Active: The computer analyzed your entire career throw history to predict your moves. Keep randomizing!"; } 
...
function resetToToss() { localStorage.removeItem('hc_tourney_boss'); location.reload(); }
function openAnalysis() { document.getElementById('analysis-modal').style.display = 'flex'; drawWormChart(); }
function closeAnalysis() { document.getElementById('analysis-modal').style.display = 'none'; }

function downloadPDF() {
    const btn = document.getElementById('pdf-btn'); if (!btn) return;
    const originalText = btn.innerHTML; btn.innerHTML = "⏳ GENERATING REPORT..."; btn.disabled = true;

    try {
        if (typeof html2pdf === 'undefined') { throw new Error("PDF Engine currently loading. Please wait a moment and click again."); }
        const pStats = gameState.playerStats; const cStats = gameState.compStats;
        const pSR = pStats.balls > 0 ? ((pStats.runs / pStats.balls) * 100).toFixed(2) : "0.00"; const cSR = cStats.balls > 0 ? ((cStats.runs / cStats.balls) * 100).toFixed(2) : "0.00";
        const pEco = cStats.balls > 0 ? (cStats.runs / (cStats.balls / 6)).toFixed(2) : "0.00"; const cEco = pStats.balls > 0 ? (pStats.runs / (pStats.balls / 6)).toFixed(2) : "0.00";
        let innStatusText = "MATCH REPORT"; const innEl = document.getElementById('innings-status'); if (innEl) { innStatusText = innEl.innerText.replace(/[🏏⚔️🏆💀🤝]/g, '').trim(); }
        ...
        let pdfHTML = `
            <div style="font-family: Arial, sans-serif; color: #000000; padding: 20px; background: #ffffff; font-size: 15px; line-height: 1.5;">
                <div style="text-align: center; border-bottom: 4px solid #000000; padding-bottom: 20px; margin-bottom: 30px;"><h1 style="font-size: 32px; font-weight: 900; color: #000000; margin: 0;">HAND CLASH</h1><h2 style="font-size: 18px; font-weight: 800; color: #000000; margin: 5px 0 0 0;">OFFICIAL MATCH REPORT</h2></div>
                <div style="text-align: center; background: #f3f4f6; padding: 15px; border-radius: 8px; margin-bottom: 30px; border-left: 8px solid #3b82f6; border: 2px solid #000000;"><h3 style="margin: 0; font-size: 20px; color: #000000; font-weight: 900;">${innStatusText}</h3></div>
                <table style="width: 100%; border-collapse: separate; border-spacing: 20px 0; margin-bottom: 30px; page-break-inside: avoid;"><tr>
                    <td style="width: 50%; vertical-align: top; background: #ffffff; border: 2px solid #000000; border-top: 8px solid #3b82f6; border-radius: 8px; padding: 20px;">
                        ...
                        <table style="width: 100%; border-collapse: collapse; font-size: 15px; color: #000000; font-weight: 700;"><tr><td style="padding: 8px 0;">Runs Scored</td><td style="text-align: right; font-weight: 900; font-size: 18px;">${pStats.runs} <span style="font-size:13px; font-weight:bold;">(${pStats.balls} balls)</span></td></tr><tr style="border-top: 1px solid #d1d5db;"><td style="padding: 8px 0;">Wickets Lost</td><td style="text-align: right; font-weight: 900; color:#dc2626;">${pStats.wicketsLost}</td></tr><tr style="border-top: 1px solid #d1d5db;"><td style="padding: 8px 0;">Strike Rate</td><td style="text-align: right; font-weight: 900;">${pSR}</td></tr><tr style="border-top: 1px solid #d1d5db;"><td style="padding: 8px 0;">Boundaries</td><td style="text-align: right; font-weight: 900;">${pStats.fours + pStats.sixes} <span style="font-size:13px; font-weight:bold;">(4s: ${pStats.fours} | 6s: ${pStats.sixes})</span></td></tr><tr style="border-top: 1px solid #d1d5db;"><td style="padding: 8px 0;">Bowling Eco.</td><td style="text-align: right; font-weight: 900;">${pEco}</td></tr><tr style="border-top: 1px solid #d1d5db;"><td style="padding: 8px 0;">Extras Rcvd.</td><td style="text-align: right; font-weight: 900;">${pStats.extras}</td></tr></table>
                    </td>
                    <td style="width: 50%; vertical-align: top; background: #ffffff; border: 2px solid #000000; border-top: 8px solid #ef4444; border-radius: 8px; padding: 20px;">
                        ...
                        <table style="width: 100%; border-collapse: collapse; font-size: 15px; color: #000000; font-weight: 700;"><tr><td style="padding: 8px 0;">Runs Scored</td><td style="text-align: right; font-weight: 900; font-size: 18px;">${cStats.runs} <span style="font-size:13px; font-weight:bold;">(${cStats.balls} balls)</span></td></tr><tr style="border-top: 1px solid #d1d5db;"><td style="padding: 8px 0;">Wickets Lost</td><td style="text-align: right; font-weight: 900; color:#dc2626;">${cStats.wicketsLost}</td></tr><tr style="border-top: 1px solid #d1d5db;"><td style="padding: 8px 0;">Strike Rate</td><td style="text-align: right; font-weight: 900;">${cSR}</td></tr><tr style="border-top: 1px solid #d1d5db;"><td style="padding: 8px 0;">Boundaries</td><td style="text-align: right; font-weight: 900;">${cStats.fours + cStats.sixes} <span style="font-size:13px; font-weight:bold;">(4s: ${cStats.fours} | 6s: ${cStats.sixes})</span></td></tr><tr style="border-top: 1px solid #d1d5db;"><td style="padding: 8px 0;">Bowling Eco.</td><td style="text-align: right; font-weight: 900;">${cEco}</td></tr><tr style="border-top: 1px solid #d1d5db;"><td style="padding: 8px 0;">Extras Rcvd.</td><td style="text-align: right; font-weight: 900;">${cStats.extras}</td></tr></table>
                    </td>
                </tr></table>
                
                ${wormImgHtml}
                
                <div style="page-break-before: auto;"><h3 style="color: #000000; font-size: 20px; font-weight: 900; border-bottom: 4px solid #000000; padding-bottom: 10px; margin-bottom: 20px;">BALL-BY-BALL MATCH LOG</h3>
                <table style="width: 100%; border-collapse: collapse; font-family: 'Courier New', Courier, monospace; font-size: 15px; line-height: 1.6; color: #000000; page-break-inside: auto;"><tbody>`;
        ...
        gameState.commentaryHistory.forEach(log => {
            ...
            if (safeText.includes("WICKET") || safeText.includes("STUMPED") || safeText.includes("HOWZAT") || safeText.includes("HIT WICKET")) { lineStyle = "margin: 4px 0; color: #b91c1c; font-weight: 900;"; } else if (safeText.includes("+4") || safeText.includes("+6")) { lineStyle = "margin: 4px 0; color: #1d4ed8; font-weight: 900;"; } else if (safeText.includes("---")) { lineStyle = "margin: 15px 0 5px 0; padding: 10px; background: #e5e7eb; border: 3px solid #000000; text-align: center; font-weight: 900; font-size: 16px;"; }
        ...
        if (currentGroup !== '') { pdfHTML += `<tr style="page-break-inside: avoid; page-break-after: auto;"><td style="border-left: 4px solid #000000; padding-left: 15px; padding-bottom: 15px; border-bottom: 1px dashed #d1d5db;">${currentGroup}</td></tr>`; }
        pdfHTML += `</tbody></table></div><div style="margin-top: 50px; text-align: center; color: #000000; font-size: 14px; font-weight: 900; border-top: 3px solid #000000; padding-top: 20px; page-break-inside: avoid;">Generated by Hand Clash Arena &bull; &copy; 2026</div></div>`;

        printElement.innerHTML = pdfHTML;
        const opt = { margin: 0.4, filename: 'Hand_Clash_Match_Report.pdf', image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: 'in', format: 'a4', orientation: 'portrait' }, pagebreak: { mode: 'css', avoid: 'tr' } };

        html2pdf().set(opt).from(printElement).save().then(() => { btn.innerHTML = originalText; btn.disabled = false; }).catch(err => { console.error("PDF engine promise caught an error:", err); btn.innerHTML = "❌ PDF ERROR"; setTimeout(() => { btn.innerHTML = originalText; btn.disabled = false; }, 3000); });
    } catch(err) { console.error("DOM access failed before PDF generation:", err); btn.innerHTML = "❌ SYSTEM ERROR"; setTimeout(() => { btn.innerHTML = originalText; btn.disabled = false; }, 3000); }
}
