/* =========================================================
   CRICPULSE FUN-GUN ARENA | ULTIMATE MASTER ENGINE
   ========================================================= */
...
const bossInfo = [
    { name: "The Rookie", icon: "🟢", color: "gray", desc: "Plays entirely randomly. A good warmup.", taunts: { wkt: ["Oops, did I do that?", "Beginner's luck!"], six: ["Wow, I hit it far!"] } },
    { name: "The Wall", icon: "🧱", color: "#00d2ff", desc: "Defends heavily. Hard to score boundaries against.", taunts: { wkt: ["Impenetrable.", "Patience is key."], six: ["A rare breach."] } },
    { name: "The Slogger", icon: "🏏", color: "orange", desc: "Highly aggressive. Throws massive numbers.", taunts: { wkt: ["A minor setback!"], six: ["OUT OF THE STADIUM!"] } },
    { name: "The Illusionist", icon: "🎭", color: "#b91c1c", desc: "Throws opposites. If you go high, he goes low.", taunts: { wkt: ["Sleight of hand!"], six: ["An illusion broke..."] } },
    { name: "The Copycat", icon: "🪞", color: "#00bfa5", desc: "Mirrors your previous move perfectly.", taunts: { wkt: ["Stop hitting yourself!"], six: ["I let you have that one."] } },
    { name: "The Gambler", icon: "🎲", color: "#facc15", desc: "Takes wild guesses. Brilliant or awful.", taunts: { wkt: ["Jackpot!"], six: ["Bust!"] } },
    { name: "The Mathematician", icon: "🧮", color: "#9333ea", desc: "Calculates the weighted average of your throws.", taunts: { wkt: ["Statistically inevitable."], six: ["A statistical anomaly."] } },
    { name: "The Sniper", icon: "🎯", color: "#ff2a2a", desc: "Reads lifetime stats. Highly predictive.", taunts: { wkt: ["Predictable.", "Right in my crosshairs."], six: ["Precision strike."] } },
    { name: "Grandmaster", icon: "🧙‍♂️", color: "#00ff88", desc: "Reads CURRENT match patterns. Learns in real-time.", taunts: { wkt: ["Checkmate.", "I am three moves ahead."], six: ["Flawless execution."] } },
    { name: "Cricket God", icon: "👑", color: "#fbbf24", desc: "The ultimate challenge. Near-perfect prediction.", taunts: { wkt: ["Bow to the God."], six: ["A mere mortal strikes?"] } }
];

const shopItems = {
    avatars: [
        { id: '👤', name: 'Default', price: 0 },
        { id: '🐯', name: 'Tiger', price: 25000 },
        { id: '👽', name: 'Alien', price: 100000 },
        { id: '🤖', name: 'Mecha', price: 500000 },
        { id: '👑', name: 'God King', price: 1000000 }
    ],
    themes: [
        { id: 'default', name: 'Cyber Green', price: 0, icon: '🟩' },
        { id: 'synthwave', name: 'Synthwave', price: 250000, icon: '🟪' },
        { id: 'blood', name: 'Blood Red', price: 750000, icon: '🟥' }
    ],
    coins: [
...
function applyCosmetics() {
    ...
    if (coinHeads && coinTails) {
        ...
        if (u.equippedCoin === 'bitcoin') {
            coinHeads.innerHTML = '₿'; coinTails.innerHTML = '₿'; 
        } else {
            coinHeads.innerHTML = 'HEADS'; coinTails.innerHTML = 'TAILS'; 
        }
    }
}
    ...
    const buildSection = (items, typeStr, unlockedArr, equippedId) => {
        ...
        items.forEach(item => {
            ...
            if (isEquipped) {
                ...
                btnHtml = `<button class="shop-btn buy" ${disabledStr} onclick="buyItem('${typeStr}', '${item.id}', ${item.price})">🪙 ${item.price.toLocaleString()}</button>`;
            }
...
function buyItem(type, itemId, price) {
    ...
    if (u.coins >= price) {
        ...
        if (cText) cText.innerText = u.coins;
        renderShop();
    }
}
...
function renderProfilePage() {
    ...
    if (rText) { rText.innerText = rankData.rank.title; rText.className = ''; rText.classList.add(rankData.rank.class); }
    
    const xText = document.getElementById('prof-xp');
    if (xText) xText.innerText = rankData.xp;
    ...
    let bowlAvg = "-";
    if (wickets > 0) bowlAvg = (stats.totalRunsConceded / wickets).toFixed(2);
    document.getElementById('prof-bowl-avg').innerText = bowlAvg;
    
    let avgSR = "0.00";
    if (stats.totalBallsFaced > 0) avgSR = ((stats.totalRuns / stats.totalBallsFaced) * 100).toFixed(2);
    
    const oversBowled = stats.totalBallsBowled / 6;
    let avgEco = "0.00";
    if (oversBowled > 0) avgEco = (stats.totalRunsConceded / oversBowled).toFixed(2);
    
    document.getElementById('prof-sr').innerText = avgSR;
    document.getElementById('prof-eco').innerText = avgEco;
    
    let bestSpellText = "-";
    if (stats.bestSpell && stats.bestSpell.wickets > 0) bestSpellText = `${stats.bestSpell.wickets}/${stats.bestSpell.runs}`;
    ...
    achievementList.forEach(ach => {
        ...
        for (let i = 0; i < ach.thresholds.length; i++) {
            if (val >= ach.thresholds[i]) { level = i + 2; } 
            else { nextTarget = ach.thresholds[i]; break; }
        }
        
        if (level > ach.thresholds.length) {
            level = 'MAX'; isMaxed = true; nextTarget = ach.thresholds[ach.thresholds.length - 1]; 
        }
    ...
    if (runsCtxElement) {
        runsChartInstance = new Chart(runsCtxElement.getContext('2d'), {
            type: 'bar', data: { labels: stats.last20Innings ? stats.last20Innings.map((inn, i) => `Wkt ${i+1}${inn.notOut ? '*' : ''}`) : [], datasets: [{ label: 'Runs Scored', data: stats.last20Innings ? stats.last20Innings.map(inn => inn.runs) : [], backgroundColor: '#00d2ff', borderRadius: 4 }] },
    ...
    if (dnaCtxElement && stats.battingThrows) {
        ...
        throwDnaInstance = new Chart(dnaCtxElement.getContext('2d'), {
            type: 'radar', data: { labels: ['1', '2', '3', '4', '5', '6', '0 (Defend)'], datasets: [{ label: 'Times Thrown', data: [t['1'], t['2'], t['3'], t['4'], t['5'], t['6'], t['0']], backgroundColor: 'rgba(0, 255, 136, 0.2)', borderColor: '#00ff88', pointBackgroundColor: '#fff', borderWidth: 2 }] },
    ...
    if (fatalCtxElement && stats.fatalThrows) {
        ...
        fatalChartInstance = new Chart(fatalCtxElement.getContext('2d'), {
            type: 'radar', data: { labels: ['1', '2', '3', '4', '5', '6', '0 (Wkt/Stmp)'], datasets: [{ label: 'Got Out On', data: [ft['1'], ft['2'], ft['3'], ft['4'], ft['5'], ft['6'], ft['0']], backgroundColor: 'rgba(255, 42, 42, 0.2)', borderColor: '#ff2a2a', pointBackgroundColor: '#fff', borderWidth: 2 }] },
            options: { plugins: { legend: { display: false } }, scales: { r: { min: 0, beginAtZero: true, angleLines: { color: 'rgba(255,255,255,0.1)' }, grid: { color: 'rgba(255,255,255,0.1)' }, pointLabels: { color: '#fff', font: {size: 14, family: "'Orbitron', sans-serif"} }, ticks: { display: false } } } }
        });
    }
}
...
function goToToss() {
    ...
    let aiMode = gameState.aiDifficulty === 'hard' ? "PRO AI" : "CASUAL AI";
    let formatMode = gameState.maxBalls === Infinity ? "CLASSIC" : `T${gameState.maxBalls/6}`;
    ...
    if (tossData.caller === 'player') {
        ...
        statusText.innerHTML = `Computer calling... <br><span style='color: var(--accent-red);'>${tossData.call.toUpperCase()}</span>`;
...
function callCoin(choice) {
    ...
    document.getElementById('toss-status-text').innerHTML = `You called: <span style='color: var(--accent-blue);'>${choice.toUpperCase()}</span>`;
    executeCoinFlip();
}
...
function continueToMatch() {
    ...
    let formatText = gameState.maxBalls === Infinity ? "Classic" : `T${gameState.maxBalls/6}`;
...
function getBossThrow(bossIndex) {
    ...
    switch (bossIndex) {
        case 0: return Math.floor(Math.random() * 6) + 1;
        case 1: if (isCompBatting) return [0, 0, 1, 1, 2, 2, 3, 4][Math.floor(Math.random() * 8)]; else return [1, 2, 3, 4, 0][Math.floor(Math.random() * 5)];
        case 2: if (isCompBatting) return [4, 4, 5, 5, 6, 6, 3][Math.floor(Math.random() * 7)]; else return [4, 5, 6, 1, 2][Math.floor(Math.random() * 5)];
        case 3: let lastT = gameState.playerHistory.length > 0 ? gameState.playerHistory[gameState.playerHistory.length - 1] : 3; return Math.max(0, Math.min(6, 6 - lastT));
        case 4: if (gameState.playerHistory.length > 0) return gameState.playerHistory[gameState.playerHistory.length - 1]; return Math.floor(Math.random() * 6) + 1;
        case 5: if (Math.random() < 0.3) return snipe(getLikely(pMatchThrows)); return Math.floor(Math.random() * 6) + 1;
        case 6: let hist = gameState.playerHistory.slice(-5); if (hist.length === 0) return Math.floor(Math.random() * 6) + 1; return snipe(Math.round(hist.reduce((a,b)=>a+b,0) / hist.length));
        case 7: if (Math.random() < 0.6) return snipe(getLikely(pLifeThrows)); return Math.floor(Math.random() * 6) + 1;
        case 8: let mLike = getLikely(pMatchThrows); if (pMatchThrows[mLike] < 3) return getComputerThrowFallback(); if (Math.random() < 0.8) return snipe(mLike); return Math.floor(Math.random() * 6) + 1;
        case 9: if (Math.random() < 0.9) return snipe(getLikely(pMatchThrows)); return Math.floor(Math.random() * 6) + 1;
...
function getComputerThrowFallback() {
    ...
    if (!isCompBatting) { 
        ...
        if (Math.random() < 0.75) return parseInt(likelyThrow); else return Math.floor(Math.random() * 7);
    } else { 
        let pBowls = stats.bowlingThrows; 
        let likelyThrow = Object.keys(pBowls).reduce((a, b) => pBowls[a] > pBowls[b] ? a : b); 
        let compNum = Math.floor(Math.random() * 7); let attempts = 0; 
...
function handleWicket(num, type) {
    ...
    let comment = "";
    if (type === 'STUMPED') comment = getRandomCommentary(commentaryDB.wkt_stumped).replace("[BATTER]", batterName);
    else if (type === 'HIT_WICKET') comment = getRandomCommentary(commentaryDB.wkt_hit).replace("[BATTER]", batterName);
    else comment = getRandomCommentary(commentaryDB.wkt_bowled).replace("[BATTER]", batterName).replace("[NUM]", num);
...
function handleWide(batterNum) {
    ...
    let comment = getRandomCommentary(commentaryDB.wide).replace("[NUM]", batterNum).replace(/\[RUNS\]/g, runsToAdd).replace(/\[TEAM\]/g, team);
...
function handleRuns(runs) {
    ...
    let comment = "";
    if (runs === 4) { currentBatterStats.fours++; comment = getRandomCommentary(commentaryDB.run_4); } 
    else if (runs === 6) { currentBatterStats.sixes++; comment = getRandomCommentary(commentaryDB.run_6); } 
    else if (runs === 5) { comment = getRandomCommentary(commentaryDB.run_5); } 
    else { comment = getRandomCommentary(commentaryDB.run_1_3).replace(/\[RUNS\]/g, runs); }
...
function drawWormChart() {
    ...
    wormChartInstance = new Chart(ctxElement.getContext('2d'), {
        ...
        options: { plugins: { legend: { labels: { color: '#fff' } } }, scales: { y: { beginAtZero: true, grid: {color: 'rgba(255,255,255,0.1)'} }, x: { title: { display: true, text: 'Balls', color: '#a1a1aa' }, grid: {color: 'rgba(255,255,255,0.1)'} } }, color: '#fff' }
    });
}

function endGame(result) {
    gameState.gameOver = true; actionArea.style.display = 'none'; 
    const endControls = document.getElementById('end-game-controls'); endControls.style.display = 'flex'; document.body.classList.remove('danger-pulse');
    ...
    if (gameState.isTournament) {
        ...
        if (playAgainBtn) {
            ...
            playAgainBtn.onclick = function() { localStorage.removeItem('hc_tourney_boss'); window.location.href = 'tournament.html'; };
        }
    }
}
...
function evaluateAchievements(stats) {
    ...
    achievementList.forEach(ach => {
        let val = ach.getVal(stats);
        let level = 1;
        for (let i = 0; i < ach.thresholds.length; i++) { if (val >= ach.thresholds[i]) { level = i + 2; } else { break; } }
        if (level > ach.thresholds.length + 1) level = 'MAX';
        
        let oldLvl = stats.achLevels[ach.id] || 1;
        if (level !== 'MAX' && level > oldLvl) {
            stats.achLevels[ach.id] = level; showToast(`🏆 LEVELED UP: ${ach.title} is now Level ${level}!`);
        } else if (level === 'MAX' && oldLvl !== 'MAX') {
            stats.achLevels[ach.id] = 'MAX'; showToast(`🌟 MAXED OUT: ${ach.title} reached MAX Level!`);
        }
    });
}
...
function saveLifetimeStats(result) {
    ...
    gameState.playerStats.dismissalHistory.forEach(d => { stats.totalDismissals += 1; stats.fatalThrows[d.num] = (stats.fatalThrows[d.num] || 0) + 1; });
    
    if (gameState.compStats.runs === 0 && gameState.compStats.wicketsLost > 0) stats.careerSnipes = (stats.careerSnipes || 0) + 1;

    gameState.playerStats.wicketRunsHistory.forEach(w => { if (w.runs === 0) stats.ducks += 1; stats.last20Innings.push({ runs: w.runs, notOut: false }); });
    
    gameState.compStats.wicketRunsHistory.forEach(w => { if (w.runs === 0) stats.aiDucksGivens = (stats.aiDucksGivens || 0) + 1; });
    
    if (gameState.playerStats.wicketsLost < gameState.maxWickets && gameState.playerStats.balls > 0) { 
        stats.notOutMatches += 1; stats.last20Innings.push({ runs: gameState.playerStats.currentWicketRuns, notOut: true }); 
    }
...
function openAnalysis() { 
    ...
    if (modal) { modal.style.display = 'flex'; drawWormChart(); }
}
    ...
    try {
        ...
        if (wormCanvas) {
            try {
                ...
                wormImgHtml = `<div style="page-break-inside: avoid; text-align: center; margin-top: 30px; margin-bottom: 30px;"><h3 style="color: #000000; font-size: 18px; border-bottom: 2px solid #000000; padding-bottom: 10px; font-weight: 900;">MATCH WORM CHART</h3><img src="${wormDataUrl}" style="width: 100%; max-width: 600px; height: auto; border: 2px solid #000; border-radius: 8px;"></div>`;
        ...
        .catch(err => { console.error("PDF engine promise caught an error:", err); btn.innerHTML = "❌ PDF ERROR"; setTimeout(() => { btn.innerHTML = originalText; btn.disabled = false; }, 3000); });
...
function switchTab(tabId) {
    ...
    const eventObj = window.event; if (eventObj && eventObj.target) { eventObj.target.classList.add('active-tab'); }
    const activeContent = document.getElementById(tabId); if (activeContent) { activeContent.classList.add('active-content'); }
}
